...
**Domain:** Rust CLI + TUI for tracking multi-provider LLM-CLI session usage (Claude Code / Codex CLI / Gemini CLI)
**Researched:** 2026-05-22
**Confidence:** HIGH (versions verified via GitHub release API, docs.rs, and current vendor sources; only a handful of MEDIUM-confidence items noted inline)
...
```bash
# Bootstrap the crate
cargo new --bin ai-hp-bar
cd ai-hp-bar

# Core
cargo add clap --features derive
cargo add ratatui                  # crossterm comes via re-export
cargo add tokio --features rt-multi-thread,macros,fs,time,signal
cargo add reqwest --no-default-features --features rustls-tls,cookies,json,gzip
cargo add rusqlite --features bundled
cargo add serde --features derive
cargo add serde_json
cargo add toml
cargo add jiff
cargo add keyring-core             # NOT `keyring` v4
...
# One-time tooling
cargo install cargo-dist          # release engineering
cargo install cargo-deny          # license/duplicate audit
cargo install cargo-nextest       # fast test runner (optional)
```

`reqwest` is intentionally `--no-default-features` to drop OpenSSL — keeps the static binary clean.
...
**If Gemini auth turns out to be a plain Bearer token (no cookies, no redirects):**
...
**If the binary size budget gets tight (< 5MB target):**
...
1. **Codex schema** — `state_5.sqlite` name encodes version `5`. When Codex moves to `state_6.sqlite` we break. *Mitigation:* probe `glob("~/.codex/state_*.sqlite")` and pick the highest; gate every query on `PRAGMA table_info` schema check.
2. **Gemini endpoint** — `gemini.google.com/usage` is undocumented. *Mitigation:* HTML/JSON response parsing isolated in `providers/gemini/scrape.rs` with snapshot fixtures; expect to maintain.
3. **keyring-core 1.0 API churn** — just hit 1.0, but the surrounding ecosystem (`keyring-secret-service`, `keyring-windows-credentials` connector crates) is still settling. *Mitigation:* abstract behind `trait SecretStore` so we can swap to file-backed in one place.
4. **jiff pre-1.0** — minor breakages possible through 2026. *Mitigation:* `Clock` trait wrapping all timezone math.
5. **ratatui 0.x** — every minor (0.29 → 0.30) has had breaking changes. *Mitigation:* `cargo upgrade --incompatible` quarterly, with snapshot tests catching layout regressions immediately.
6. **cargo-dist pre-1.0** — being renamed to `dist` eventually. *Mitigation:* the `Cargo.toml` config block is small and trivially portable.
...
*Stack research for: AHB (AI HP Bar) — Rust CLI + TUI usage tracker*
*Researched: 2026-05-22*
